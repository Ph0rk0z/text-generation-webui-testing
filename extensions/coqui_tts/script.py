...
#    'voice_clone_reference_path': '/home/mint/gptq2/text-generation-webui-testing/extensions/tortoise_tts/tortoise/tortoise/voices/demondice/4.wav',
#    'voice_clone_reference_path': '/home/mint/gptq2/text-generation-webui-testing/extensions/tortoise_tts/tortoise/tortoise/voices/biden/15.wav',
    'voice_clone_reference_path': '/home/mint/tortoise-tts-fast/tortoise/voices/nyan/2.wav',
...
#    paths = tts.speaker_wav if tts.speaker_wav is not None else []
#    temp_path = params['voice_clone_reference_path'] if params['voice_clone_reference_path'] in paths else paths[0] if len(paths) > 0 else None

    return tts, temp_speaker, temp_language#, temp_path
...
def ui():
    ...
    with gr.Accordion("Coqui AI TTS"):
        ...
        path = gr.Textbox(value=params['voice_clone_reference_path'], choices=[], label='Path')
    ...
    path.change(lambda x: params.update({"voice_clone_reference_path": x}), path, None)
...
def update_model(x, speaker_dropdown, language_dropdown):
    ...
    path_dropdown.update(value=path, choices=paths)
