...
group.add_argument('--loader', type=str, help='Choose the model loader manually, otherwise, it will get autodetected. Valid options: Transformers, llama.cpp, llamacpp_HF, ExLlamav2_HF, ExLlamav2, ExLlama_HF, ExLlama, AutoGPTQ, AutoAWQ, GPTQ-for-LLaMa, QuIP#.')
...
group.add_argument('--threshold', type=float, default=1.5, help='8 bit threshold for older cards')
...
group.add_argument('--flash-attn', action='store_true', help='Use flash-attention. llama.cpp')
...
group.add_argument('--autogptq', action='store_true', help='Enable AutoGPTQ.')
group.add_argument('--triton', action='store_true', help='Use triton.')
group.add_argument('--autogptq_device_map', type=str, default='auto', help='Device map for AutoGPTQ. e.g. auto')
group.add_argument('--autogptq_act_order', action='store_true', help='Act-order or desc_act for AutoGPTQ. Use if you have group size and act order together')
...
group.add_argument('--gptq-for-llama', action='store_true', help='Classic GPTQ implementation.')
...
group.add_argument('--autograd', action='store_true', default=False, help='Use the autograd GPTQ loader for llama and llama lora')
group.add_argument('--v1', action='store_true', default=False, help='Explicity declare GPTQv1 Model to Autograd')
group.add_argument('--warmup_autotune', action='store_true', help='(triton) Enable warmup autotune.')
group.add_argument('--quant_attn', action='store_true', help='(Exllama/Autograd/GPTQ-triton/cuda-autogptq) Enable Fused Attention.')
group.add_argument('--fused_mlp', action='store_true', help='(Exllama/Autograd/AutoGPTQ-triton) Enable Fused MLP.')
group.add_argument('--xformers', action='store_true', help="Use xformer's memory efficient attention. This should increase your tokens/s.")
